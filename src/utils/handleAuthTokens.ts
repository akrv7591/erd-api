...
export default async function handleAuthTokens(req: any, res: express.Response, user: User, transaction: Transaction | null = null) {
  ...
  // refresh token exists in the database and belongs to the current user-router than we need to delete it
  // if the token does not belong to the current user-router, then we delete all refresh tokens
  ...
  if (cookies?.[config.jwt.refresh_token.cookie_name]) {
    // check if the given refresh token is from the current user-router
    ...
    // if this token does not exists int the database or belongs to another user-router,
